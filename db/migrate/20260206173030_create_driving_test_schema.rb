...
class CreateDrivingTestSchema < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :questions, %i[question_bank_id official_number], unique: true
    add_index :questions, %i[scope question_weight active]
    ...
    add_index :question_translations, %i[question_id locale], unique: true
    ...
    add_index :question_options, %i[question_id key], unique: true
    add_index :question_options, %i[question_id position], unique: true
    ...
    add_index :question_option_translations, %i[question_option_id locale], unique: true
    ...
    add_index :question_categories, %i[question_id license_category_id], unique: true
    ...
    add_index :question_media_links, %i[question_id slot], unique: true
    add_index :question_media_links, :source_filename
    add_index :question_media_links, %i[media_asset_id slot]
    ...
    add_index :exam_blueprint_rules, %i[exam_blueprint_id scope question_weight], unique: true
    ...
    add_index :exam_attempt_items, %i[exam_attempt_id position], unique: true
    add_index :exam_attempt_items, %i[exam_attempt_id question_id], unique: true
    add_index :exam_attempt_items, %i[exam_attempt_id answered_correctly]
    ...
    add_index :import_issues, %i[import_run_id severity]
    ...
    add_check_constraint :questions, 'question_weight IS NULL OR question_weight IN (1, 2, 3)',
                         name: 'questions_weight_valid'
    add_check_constraint :questions,
                         "(answer_mode = 0 AND correct_key IN ('T', 'N')) OR (answer_mode = 1 AND correct_key IN ('A', 'B', 'C'))", name: 'questions_correct_key_matches_mode'

    add_check_constraint :question_translations, "locale IN ('pl', 'en', 'de', 'ua')",
                         name: 'question_translations_locale_valid'
    add_check_constraint :question_option_translations, "locale IN ('pl', 'en', 'de', 'ua')",
                         name: 'question_option_translations_locale_valid'
    ...
    add_check_constraint :media_assets, 'processing_status IN (0, 1, 2, 3)',
                         name: 'media_assets_processing_status_valid'
    add_check_constraint :media_assets, 'byte_size IS NULL OR byte_size >= 0',
                         name: 'media_assets_byte_size_non_negative'
    add_check_constraint :media_assets, 'duration_ms IS NULL OR duration_ms >= 0',
                         name: 'media_assets_duration_non_negative'
    ...
    add_check_constraint :exam_blueprints, 'specialist_questions_count > 0',
                         name: 'exam_blueprints_specialist_count_positive'
    ...
    add_check_constraint :exam_blueprints, 'questions_total = basic_questions_count + specialist_questions_count',
                         name: 'exam_blueprints_total_matches_parts'
    add_check_constraint :exam_blueprints,
                         'effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from', name: 'exam_blueprints_effective_dates_order'

    add_check_constraint :exam_blueprint_rules, 'scope IN (0, 1)', name: 'exam_blueprint_rules_scope_valid'
    add_check_constraint :exam_blueprint_rules, 'question_weight IN (1, 2, 3)',
                         name: 'exam_blueprint_rules_weight_valid'
    add_check_constraint :exam_blueprint_rules, 'questions_count > 0',
                         name: 'exam_blueprint_rules_questions_count_positive'
    ...
    add_check_constraint :exam_attempts, 'score IS NULL OR (score >= 0 AND score <= max_score)',
                         name: 'exam_attempts_score_valid_range'
    ...
    add_check_constraint :exam_attempt_items, 'points_possible IN (1, 2, 3)',
                         name: 'exam_attempt_items_points_possible_valid'
    add_check_constraint :exam_attempt_items, "selected_key IS NULL OR selected_key IN ('T', 'N', 'A', 'B', 'C')",
                         name: 'exam_attempt_items_selected_key_valid'
    add_check_constraint :exam_attempt_items, "correct_key IN ('T', 'N', 'A', 'B', 'C')",
                         name: 'exam_attempt_items_correct_key_valid'
    ...
    add_check_constraint :import_runs, 'finished_at IS NULL OR finished_at >= started_at',
                         name: 'import_runs_finished_after_start'

    add_check_constraint :import_issues, 'severity IN (0, 1)', name: 'import_issues_severity_valid'
    add_check_constraint :import_issues, 'row_number IS NULL OR row_number > 0',
                         name: 'import_issues_row_number_positive'
  end
end
